...
// Generated by mro at 2021-07-16 11:21:27
package postgres
...
func (update *Update) UpdateXuByIdAndAge(user *models.User) error {
	...
	var1 := dbutil425.FormatColumnTime(update.p.GetTimeStampzFormat(),
...
func (update *Update) UpdateByName(user *models.User, gender bool) (int64, error) {
	...
	var var3tp *dbutil425.TimeProp
	var3tp, err = dbutil425.NewTimeProp("DATETIME", true, "")
	if err != nil {
		return 0, err
	}
	var3 := dbutil425.FormatColumnTime(update.p.GetTimeStampzFormat(),
		update.p.GetTimezone(),
		var3tp,
		user.Cret)
...
func (update *Update) Update2Name(user models.User, ids []uint64) (int64, error) {
	...
	var3 := dbutil425.FormatColumnTime(update.p.GetTimeStampzFormat(),
		...
		var3tp,
		user.Cret)
	sql := "UPDATE user SET name=?,age=?,gender=?,oka_us_url=?,cret=?,url=? WHERE name=? or id in (@in-1626405688)"
	sqlParams := []interface{}{user.Name, user.Age, user.Gender, user.OkaUsURL, var3, user.URL}
	// where 参数
	sqlParams = append(sqlParams, user.Name)
	if len(ids) <= 0 {
		return 0, dbutil425.NewErrParamIsEmpty("ids")
	}
	sql = dbutil425.TakePlaceHolder(sql, "@in-1626405688", len(ids))
