...
// Generated by mro at 2021-07-16 16:48:21
package postgres
...
func (update *Update) UpdateXuByIdAndAge(user *models.User) error {
	...
	var var1tp *dbutil540.TimeProp
	var1tp, err = dbutil540.NewTimeProp("DATETIME", true, "")
	if err != nil {
		return err
	}
	var1 := dbutil540.FormatColumnTime(update.p.GetTimeStampzFormat(),
...
func (update *Update) UpdateByName(user *models.User, gender bool) (int64, error) {
	...
	var3 := dbutil540.FormatColumnTime(update.p.GetTimeStampzFormat(),
		update.p.GetTimezone(),
		var3tp,
		user.Cret)
...
func (update *Update) Update2Name(user models.User, ids []uint64) (int64, error) {
	...
	var3 := dbutil540.FormatColumnTime(update.p.GetTimeStampzFormat(),
		...
		var3tp,
		user.Cret)
	sql := "UPDATE user SET name=?,age=?,gender=?,oka_us_url=?,cret=?,url=? WHERE name=? or id in (@in-1626425302)"
	sqlParams := []interface{}{user.Name, user.Age, user.Gender, user.OkaUsURL, var3, user.URL}
	// where 参数
	sqlParams = append(sqlParams, user.Name)
	if len(ids) <= 0 {
		return 0, dbutil540.NewErrParamIsEmpty("ids")
	}
	sql = dbutil540.TakePlaceHolder(sql, "@in-1626425302", len(ids))
