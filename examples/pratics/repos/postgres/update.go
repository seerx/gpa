...
// Generated by mro at 2021-07-19 17:36:20
package postgres
...
func (update *Update) UpdateXuByIdAndAge(user *models.User) error {
	...
	var1 := dbutil274.FormatColumnTime(update.p.GetTimeStampzFormat(),
...
func (update *Update) UpdateByName(user *models.User, gender bool) (int64, error) {
	...
	var3 := dbutil274.FormatColumnTime(update.p.GetTimeStampzFormat(),
		update.p.GetTimezone(),
		var3tp,
		user.Cret)
...
func (update *Update) Update2Name(user models.User, ids []uint64) (int64, error) {
	...
	var3 := dbutil274.FormatColumnTime(update.p.GetTimeStampzFormat(),
		...
		var3tp,
		user.Cret)
	sql := "UPDATE user SET name=?,age=?,gender=?,oka_us_url=?,cret=?,url=? WHERE name=? or id in (@in-1626687381)"
	sqlParams := []interface{}{user.Name, user.Age, user.Gender, user.OkaUsURL, var3, user.URL}
	// where 参数
	sqlParams = append(sqlParams, user.Name)
	if len(ids) <= 0 {
		return 0, dbutil274.NewErrParamIsEmpty("ids")
	}
	sql = dbutil274.TakePlaceHolder(sql, "@in-1626687381", len(ids))
