...
import (
	context "context"
	dbutil300 "github.com/seerx/gpa/rt/dbutil"
...
func (find *Find) FindA(ctx context.Context, id uint64) (*models.User, error){
    ...
    var1 := &models.User{}
    err = var2.Scan(&var1.ID, &var1.Name, &var1.Age)
    if err != nil {
        return nil, err
    }
...
func (find *Find) FindUsers1(ids []uint64) ([]*models.User, error){
    var err error
	sql := "SELECT id,name,age,gender,oka_us_url,cret,url FROM user WHERE id in (@in-1626428310)"
	var sqlParams []interface{}
	// where 参数
	if len(ids) <= 0 {
		return nil, dbutil300.NewErrParamIsEmpty("ids")
	}
	sql = dbutil300.TakePlaceHolder(sql, "@in-1626428310", len(ids))
