//+mro-ignore
// DO NOT EDIT THIS FILE
// Generated by mro at 2021-07-21 09:56:46
package postgres

import (
	context "context"
	dbutil89 "github.com/seerx/gpa/rt/dbutil"
	models "github.com/seerx/gpa/examples/pratics/models"
	rt81 "github.com/seerx/gpa/rt"
)

type UserRepo struct {
	p *rt81.Provider
}

func (userRepo *UserRepo) Insert(ctx context.Context, user *models.User, name string) (*models.User, error) {
	user.Name = name
	var err error
	var var2tp *dbutil89.TimeProp
	var2tp, err = dbutil89.NewTimeProp("DATETIME", true, "")
	if err != nil {
		return nil, err
	}
	var2 := dbutil89.FormatColumnTime(userRepo.p.GetTimeStampzFormat(),
		userRepo.p.GetTimezone(),
		var2tp,
		user.Cret)
	var1 := "INSERT INTO user (id,name,age,gender,oka_us_url,cret,url) VALUES (?,?,?,?,?,?,?)"
	_, err = userRepo.p.Executor().ExecContext(ctx, var1, user.ID, user.Name, user.Age, user.Gender, user.OkaUsURL, var2, user.URL)
	if err != nil {
		return nil, err
	}
	
	return user, err 
}

func (userRepo *UserRepo) InsertA(user *models.User) error {
	var err error
	var var2tp *dbutil89.TimeProp
	var2tp, err = dbutil89.NewTimeProp("DATETIME", true, "")
	if err != nil {
		return err
	}
	var2 := dbutil89.FormatColumnTime(userRepo.p.GetTimeStampzFormat(),
		userRepo.p.GetTimezone(),
		var2tp,
		user.Cret)
	var1 := "INSERT INTO user (id,name,age,gender,oka_us_url,cret,url) VALUES (?,?,?,?,?,?,?)"
	_, err = userRepo.p.Executor().Exec(var1, user.ID, user.Name, user.Age, user.Gender, user.OkaUsURL, var2, user.URL)
	if err != nil {
		return err
	}
	
	return err 
}

func (userRepo *UserRepo) InsertB(user *models.User, DD string, URL string) (models.User, error) {
	user.URL = URL
	var err error
	var var2tp *dbutil89.TimeProp
	var2tp, err = dbutil89.NewTimeProp("DATETIME", true, "")
	if err != nil {
		return models.User{}, err
	}
	var2 := dbutil89.FormatColumnTime(userRepo.p.GetTimeStampzFormat(),
		userRepo.p.GetTimezone(),
		var2tp,
		user.Cret)
	var1 := "INSERT INTO user (id,name,age,gender,oka_us_url,cret,url) VALUES (?,?,?,?,?,?,?)"
	_, err = userRepo.p.Executor().Exec(var1, user.ID, user.Name, user.Age, user.Gender, user.OkaUsURL, var2, user.URL)
	if err != nil {
		return models.User{}, err
	}
	
	return *user, err 
}

func (userRepo *UserRepo) InsertC(user *models.User) (*models.User, error) {
	var err error
	var var2tp *dbutil89.TimeProp
	var2tp, err = dbutil89.NewTimeProp("DATETIME", true, "")
	if err != nil {
		return nil, err
	}
	var2 := dbutil89.FormatColumnTime(userRepo.p.GetTimeStampzFormat(),
		userRepo.p.GetTimezone(),
		var2tp,
		user.Cret)
	var1 := "INSERT INTO user (id,name,age,gender,oka_us_url,cret,url) VALUES (?,?,?,?,?,?,?)"
	_, err = userRepo.p.Executor().Exec(var1, user.ID, user.Name, user.Age, user.Gender, user.OkaUsURL, var2, user.URL)
	if err != nil {
		return nil, err
	}
	
	return user, err 
}

func (userRepo *UserRepo) InsertUser(name string) (*models.User, error) {
	var2 := &models.User{}
	var2.Name = name
	var err error
	var1 := "INSERT INTO user (name) VALUES (?)"
	_, err = userRepo.p.Executor().Exec(var1, name)
	if err != nil {
		return nil, err
	}
	
	return var2, err 
}
